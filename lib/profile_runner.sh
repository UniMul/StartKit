#!/usr/bin/env bash
profile_reset_context() { unset PROFILE_NAME PROFILE_DESCRIPTION PROFILE_MODULES OPTIONAL_MODULES; }
profile_load() {
  local profile_name="$1"
  local profile_path; profile_path="$(resolve_profile "$profile_name")" || fail "profile not found: ${profile_name}"
  profile_reset_context
  # shellcheck disable=SC1090
  source "$profile_path"
  PROFILE_NAME="${PROFILE_NAME:-$profile_name}"
  PROFILE_DESCRIPTION="${PROFILE_DESCRIPTION:-}"
  declare -ag PROFILE_MODULES=("${PROFILE_MODULES[@]:-}")
  declare -ag OPTIONAL_MODULES=("${OPTIONAL_MODULES[@]:-}")
}
run_profile_install() {
  local profile_name="$1"
